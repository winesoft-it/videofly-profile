...
HLS playlist, 오디오/비디오 인코딩, encrypt, composition 같은 동작은 profile 값으로 조정한다.

## Profile 기본 구조

Profile은 공통 설정과 `renditions`로 구성한다.
`renditions`는 실제로 제공할 출력 목록이며, 배열 순서는 `master.m3u8`의 출력 순서로 사용할 수 있다.
...
| `hls` | HLS playlist, segment, 트랜스코딩 정책을 설정한다. |
| `renditions` | 제공할 출력 rendition 목록이다. |

## 기본값 파일

`schemes/v1/profile.default.json`은 profile v1의 기본값 기준 파일이다.
profile에 설정값이 없으면 이 파일에 정의된 값을 기본값으로 적용한다.
...
| 설정 | 값 | 설명 |
| --- | --- | --- |
| `transcode.policy` | `always`, `if_bitrate_exceeds`, `if_spec_mismatch`, `if_codec_mismatch` | 트랜스코딩 여부 정책이다. |
| `transcode.upscale` | boolean | 원본보다 큰 해상도로 트랜스코딩할지 여부이다. |
...
`hls.transcode`는 원본 영상을 기준으로 트랜스코딩할지 결정한다.
각 `policy`는 rendition별로 적용된다.
...
| 값 | 동작 | 예시 |
| --- | --- | --- |
| `loose` | 원본 키프레임을 위배하지 않는 범위에서 분할한다. | `segment_duration = 2`여도 원본 키프레임이 2.4초 지점에 있으면 해당 지점 기준으로 segment가 나뉠 수 있다. |
| `balance` | `segment_duration` 기준 +/- 1초 범위에서 가능한 적게 분할한다. | `segment_duration = 2`이면 2초, 4초, 6초 분할점을 기준으로 각각 1~3초, 3~5초, 5~7초 범위 안의 키프레임을 찾고, 있으면 해당 키프레임에 맞춰 분할한다. |
| `strict` | `segment_duration` 단위에 맞춰 분할한다. | `segment_duration = 2`이면 0~2초, 2~4초, 4~6초처럼 2초 단위 분할을 기준으로 한다. |
