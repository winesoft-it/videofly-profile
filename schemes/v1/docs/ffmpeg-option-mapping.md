...
이 문서는 VideoFly profile v1의 profile/rendition 값이 이 프로젝트에서 FFmpeg 실행 옵션으로 변환되는 기준을 정리한다.
목표는 FFmpeg 전체 옵션 목록을 만드는 것이 아니라, schema에 노출된 값이 어떤 FFmpeg 옵션으로 이어지는지 정의하는 것이다.
FFmpeg 옵션과 1:1로 대응하지 않는 값은 변환 단계에서의 사용 범위만 명시한다.
...
| `hls.segment_duration` | sec | HLS 산출물의 목표 segment 길이로 사용 |
| `playout.items[].duration` | sec | playout asset 재생 길이 정책으로 사용 |

## HLS 기본 처리

HLS 출력 생성은 rendition의 `container`와 profile의 `hls` 정책을 기준으로 구성한다.
이 섹션은 HLS muxer에 전달되는 변환 옵션만 다룬다.
master playlist, media playlist, segment 산출물 구성은 `hls-output-structure.md`에서 다룬다.
...
| HLS muxer segment time | `-hls_time 36000` | 기본 FFmpeg 출력은 단일 요청 output 생성을 기준으로 구성한다. `hls.segment_duration`과 1:1로 매핑하지 않는다. |
